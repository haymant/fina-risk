...
def main() -> None:
    ...
    expiry = max(int(j["commonData"]["dealData"].get("expiryDate", j["commonData"]["dealData"].get("maturityDate"))) for j in root["Chunk"]["Jobs"])
    dates = weekday_serials(market.evaluation_date, expiry)
    steps = len(dates)
    rng = np.random.default_rng(args.seed)
    z1 = rng.standard_normal((args.paths, steps))
    z2 = market.correlation * z1 + np.sqrt(max(1.0 - market.correlation**2, 0.0)) * rng.standard_normal((args.paths, steps))
    dt = 1.0 / 252.0
    shocks = np.stack((z1, z2), axis=2)
    increments = (market.rate - 0.5 * market.vols**2)[None, None, :] * dt + market.vols[None, None, :] * np.sqrt(dt) * shocks
    ...
    Path(args.output + ".meta.json").write_text(json.dumps({"paths": args.paths, "observations": steps, "underlyings": 2, "seed": args.seed, "dates": dates.tolist()}, indent=2) + "\n")
    print(json.dumps({"output": args.output, "paths": args.paths, "observations": steps, "dates": dates.tolist()}, indent=2))
