...
from fina_risk.daily_termsheet import price_daily_termsheet
...
def main() -> None:
    ...
    spots = np.fromfile(args.daily_paths, dtype=np.float64).reshape(meta["paths"], meta["observations"], meta["underlyings"])
    ...
    for i in range(spots.shape[2]):
        up = spots.copy(); down = spots.copy()
        up[:, :, i] *= 1.01; down[:, :, i] *= 0.99
        pu, pd = canonical(up), canonical(down)
        deltas.append((pu - pd) / (0.02 * 1.0))
        gammas.append((pu - 2.0 * canonical_pv + pd) / 0.01**2)
    print(json.dumps({"engine": "python_daily_termsheet", "daily_observations": first.daily_observations, "observation_dates": first.observation_dates, "job_pvs": [r.pv for r in results], "job_put_prices": [r.put_price for r in results], "job_coupon_pvs": [r.coupon_pv for r in results], "pv": canonical_pv, "put_price": first.put_price, "coupon_pv": coupon, "relative_delta": deltas, "relative_gamma": gammas, "ki_probability": first.ki_probability, "ko_probability": first.ko_probability, "coupon_fixings": results[2].coupon_fixings if len(results) >= 3 else first.coupon_fixings, "memory_carry": results[2].memory_carry if len(results) >= 3 else first.memory_carry}, indent=2))
