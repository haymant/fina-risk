...
int main(int argc, char** argv) {
    if (argc < 5) { std::cerr << "usage: aad_benchmark instruments.json market.json paths.bin paths [bump]\n"; return 2; }
...
#pragma omp parallel for schedule(static) reduction(+:pv_sum,aad_delta_sum,aad_delta_dollar_sum,hybrid_delta_sum,hybrid_delta_dollar_sum,fallback_count)
    for (std::int64_t ti=0; ti<static_cast<std::int64_t>(trades.size()); ++ti) {
        ...
        for(std::size_t k=0;k<mult.size();++k){
            const double aad_d=derivative(mult[k])/spots[trade.indices[k]];
            ...
            const bool transition=std::abs(fd_d-aad_d)>1e-10*std::max(1.0,std::abs(fd_d));
            const double d=transition?fd_d:aad_d;
            aad_delta_sum += aad_d; aad_delta_dollar_sum += aad_d*spots[trade.indices[k]]; hybrid_delta_sum += d; hybrid_delta_dollar_sum += d*spots[trade.indices[k]]; if(transition) ++fallback_count;
        }
    }
    ...
    std::cout << json{{"backend","XAD_reverse_mode_AAD"},{"quantlib_discount_factor",discount},{"instruments",trades.size()},{"underlyings",n},{"paths",paths},{"pv_checksum",pv_sum},{"aad_delta_checksum",aad_delta_sum},{"aad_delta_dollar_checksum",aad_delta_dollar_sum},{"hybrid_delta_checksum",hybrid_delta_sum},{"hybrid_delta_dollar_checksum",hybrid_delta_dollar_sum},{"fallback_count",fallback_count},{"elapsed_seconds",elapsed},{"instruments_per_second",trades.size()/elapsed},{"shared_path_cube",argv[3]},{"transition_policy","fixed-branch XAD AAD; CRN bump/revalue fallback when transition-sensitive"}}.dump(2)<<'\n';
}
