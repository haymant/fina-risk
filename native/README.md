...
Wheel tags are built for cp312 across manylinux, pair published by the
repo tag-triggered GitHub Action (TestPyPI then PyPI) as the `fina-risk`
distribution (import module `fina_risk_cpp`), mirroring `fina-core`.